...
SpriteSheet::SpriteSheet(GLuint textureBufferID, int columnCount, int rowCount) {
...
void SpriteSheet::ReloadTextCoordinates() {
    int maxIndex = columnCount * rowCount - 1;
    int columnPercent = 1.0f / (float)columnCount; //1/2 = .5%, so 0%/50% or 0/1 out of 2
    ...
    int rowPercent = 1.0f / (float)rowCount;
